...
const WRITER_ACTIONS_BASE = [
  { name: "data-driven", icon: "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z", labelKey: "dataDriven", prompt: "Rewrite this slide content to be more data-driven. Replace vague statements with specific numbers, percentages, and metrics from the source data. Every bullet should contain at least one concrete data point." },
  { name: "comparison", icon: "M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4", labelKey: "comparison", prompt: "Add comparison elements to this slide content. Compare current vs previous period, target vs actual, or before vs after. Use delta values (e.g., +15%, -2.3 points) to highlight changes." },
  { name: "trends", icon: "M13 7h8m0 0v8m0-8l-8 8-4-4-6 6", labelKey: "trends", prompt: "Refocus this slide content on trends and patterns. Highlight what is improving, declining, or stable. Use directional language (increased, decreased, remained flat)." },
  ...
  { name: "executive", icon: "M12 14l9-5-9-5-9 5 9 5zm0 0l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z", labelKey: "executiveTone", prompt: "Rewrite this slide content in a more executive tone. Lead with conclusions, not details. Use authoritative language. Remove operational jargon. Every point should connect to a business outcome or decision." },
  { name: "shorter", icon: "M20 12H4", labelKey: "shorter", prompt: "Cut this slide content by 40-50%. Keep only the most impactful points. Maximum 3-4 bullets. Remove supporting details and keep conclusions." },
  { name: "longer", icon: "M12 4v16m8-8H4", labelKey: "longer", prompt: "Expand this slide with more depth. Add supporting evidence, context, and specific examples from the data. Aim for 5-7 bullets." },
  { name: "simplify", icon: "M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z", labelKey: "simplify", prompt: "Simplify this slide content. Use shorter sentences, simpler words, and clearer structure. Remove acronyms or spell them out. A non-expert should understand every point." },
];
...
export function SlideContentCard({ slide, index, expanded, onToggle, onUpdate, onRefine, onRequestAlternatives, saving, saved, refining }: Props) {
  ...
  const bodyContent: string[] = (body.content || []).map((item: any) => typeof item === "string" ? item : typeof item === "object" ? JSON.stringify(item) : String(item));
  ...
  return (
    <div className={`card overflow-hidden transition-all duration-200 ${refining ? "opacity-70" : ""}`} dir={isRTL ? "rtl" : "ltr"}>
      {/* Header */}
      <div className="flex items-center gap-3 px-5 py-4 cursor-pointer" onClick={onToggle}>
        ...
        <span className={`badge shrink-0 text-[10px] uppercase tracking-wider ${typeColor}`}>
          {SLIDE_TYPE_KEYS[slideType] ? t(SLIDE_TYPE_KEYS[slideType]) : slideType.replace("_", " ")}
        </span>
      ...
      {expanded && (
        <div className="border-t border-gray-100 px-5 py-4 space-y-5 animate-fade-in" onClick={(e) => e.stopPropagation()}>
          {/* Body content */}
          <div>
            <label className="mb-2 block text-[10px] font-semibold uppercase tracking-wider text-gray-400">{t("content")}</label>
            {bodyType === "bullets" ? (
          ...
          {cj.key_takeaway !== undefined && (
            <div>
              <label className="mb-2 block text-[10px] font-semibold uppercase tracking-wider text-gray-400">{t("keyTakeaway")}</label>
              <div className={`rounded-lg bg-blue-50/30 p-3 ${isRTL ? "border-r-2 border-l-0" : "border-l-2 border-r-0"} border-[#0091DA]`}>
                <EditableText
          ...
          {cj.data_table && cj.data_table.headers && (
            <div>
              <label className="mb-2 block text-[10px] font-semibold uppercase tracking-wider text-gray-400">{t("dataTable")}</label>
              <div className="overflow-x-auto rounded-lg border border-gray-200">
                <table className="min-w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50">
                      {cj.data_table.headers.map((h: string, i: number) => (
                        <th key={i} className={`px-3 py-2 text-xs font-semibold uppercase tracking-wider text-gray-500 ${isRTL ? "text-right" : "text-left"}`}>{h}</th>
                      ))}
                    </tr>
                  </thead>
          ...
          {cj.chart_data && (
            <div>
              <label className="mb-2 block text-[10px] font-semibold uppercase tracking-wider text-gray-400">{t("chartSuggestion")}</label>
              <div className="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm">
                ...
                <span className="font-medium text-gray-700">
                  {cj.chart_data.chart_type === "bar" ? t("barChart") : cj.chart_data.chart_type === "line" ? t("lineChart") : cj.chart_data.chart_type === "pie" ? t("pieChart") : cj.chart_data.chart_type === "donut" ? t("donutChart") : cj.chart_data.chart_type}
                </span>
                <span className="text-gray-400">&middot;</span>
                <span className="text-gray-500">
                  {cj.chart_data.datasets?.length || 0} {t("dataSeries")}
                </span>
              </div>
            </div>
          )}
          ...
          <div className="border-t border-gray-100 pt-3 mt-3">
            <QuickActionPills
              actions={dynamicActions.length > 0 ? dynamicActions : writerActions}
      ...
      {alternatives.length > 0 && (
        <div className="border-t border-gray-200 bg-gray-50 p-4 animate-fade-in">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm font-semibold text-gray-900">{t("chooseAlternative")}</p>
            <button onClick={() => setAlternatives([])} className="btn-ghost text-xs">{t("cancel")}</button>
          </div>
            ...
            {alternatives.map((alt, i) => {
              ...
              return (
                <button key={i} onClick={() => selectAlternative(alt)}
                  ...
                  <p className="mt-2 text-[11px] font-medium text-[#0091DA]">{t("selectVersion")}</p>
                </button>
              );
            })}
          </div>
